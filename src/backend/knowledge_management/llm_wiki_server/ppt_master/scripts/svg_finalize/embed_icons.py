...
def resolve_icon_path(icon_name: str, icons_dir: Path) -> tuple[Path, float]:
    ...
    # Fallback chain: when icon not found in requested library,
    # try same-family alternatives before giving up.
    _FALLBACK_ORDER = {
        "tabler-filled": ["tabler-outline", "chunk-filled"],
        "tabler-outline": ["tabler-filled", "chunk-filled"],
        "chunk-filled": ["tabler-filled", "tabler-outline"],
        "phosphor-duotone": ["tabler-filled", "tabler-outline"],
    }
    ...
    if '/' in icon_name:
        ...
        # Cross-library fallback: if icon not found, try alternative libraries
        if not icon_path.exists():
            for fallback_lib in _FALLBACK_ORDER.get(lib, []):
                fb_path = icons_dir / fallback_lib / f'{name}.svg'
                if fb_path.exists():
                    icon_path = fb_path
                    base_size = ICON_BASE_SIZES.get(fallback_lib, 24)
                    break
    else:
...
def parse_use_element(use_match: str) -> dict[str, str | float]:
    ...
    # Extract optional stroke color (for stroke-style libraries)
